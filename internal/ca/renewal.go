...
func (r *RenewalServer) handleRenew(w http.ResponseWriter, req *http.Request) {
	...
	fingerprint := fmt.Sprintf("%x", sha256.Sum256(peerCerts[0].Raw))

	tenant, err := r.gateway.ResolveTenant(req.Context(), auth.NewCertCredential(fingerprint))
	if err != nil {
		log.Printf("[ca] renewal tenant lookup error: %v", err)
	...
	if err != nil {
		log.Printf("[ca] renewal failed: %v", err)
	...
	if _, err := r.store.AddCertificate(req.Context(), tenant.ID,
		...
		log.Printf("[ca] store new cert metadata failed: %v", err)
