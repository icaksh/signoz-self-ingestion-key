// Package config loads and validates the process environment, reproducing the
// exact operational contract of the OTLP proxy: env-var names, secure
// defaults, listen-address precedence, and optional-subsystem preconditions.
package config
...
// Load reads the environment and returns a validated Config. Validation errors
// are fatal-level for the caller: missing required values or unreadable cert
// files cause startup to fail before any listener binds.
func Load() (*Config, error) {
	...
	// Admin listen address: ADMIN_LISTEN_ADDR takes priority over ADMIN_PORT.
	...
	// Forwarded headers (X-Forwarded-For / X-Real-IP) are NOT trusted unless
	// explicitly enabled. Default false fixes the legacy spoofable trust.
	...
	if c.SyslogEnabled || c.CAEnabled {
