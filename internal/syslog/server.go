...
// Config configures the syslog mTLS server.
type Config struct {
	Addr              string
...
// Server accepts RFC 5425 syslog frames over mTLS, stamps tenant identity,
// rate-limits, and forwards to the collector.
type Server struct {
	cfg          Config
	store        *store.Store
...
// acquireConnSlot attempts to acquire a per-tenant connection slot.
...
// ListenAndServe runs the accept loop until ctx is cancelled, then joins all
// connection goroutines and closes the collector pool.
